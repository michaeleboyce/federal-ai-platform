...
async function seedProducts() {
  ...
  for (const product of productsData) {
    try {
      ...
      errors++;
      if (errors < 5) console.error(`   Error inserting product ${product.fedramp_id}:`, error.message);
    }
  }
...
async function seedAIServices() {
  ...
  const servicesData = JSON.parse(
    fs.readFileSync(path.join(DATA_DIR, 'ai_service_analysis.json'), 'utf-8')
  );
  ...
  for (const service of servicesData) {
    try {
      ...
      errors++;
      if (errors < 5) console.error(`   Error inserting AI service:`, error.message);
    }
  }
...
async function seedAgencies() {
  ...
  const agenciesData = JSON.parse(
    fs.readFileSync(path.join(DATA_DIR, 'agency_ai_usage.json'), 'utf-8')
  );
  ...
  for (const agency of agenciesData) {
    try {
      ...
      if (!globalThis.agencyIdMap) globalThis.agencyIdMap = {};
      globalThis.agencyIdMap[agency.id] = result[0].id;

      count++;
    } catch (error: any) {
      errors++;
      if (errors < 5) console.error(`   Error inserting agency:`, error.message);
    }
  }
...
async function seedAgencyMatches() {
  ...
  const matchesData = JSON.parse(
    fs.readFileSync(path.join(DATA_DIR, 'agency_service_matches.json'), 'utf-8')
  );
  ...
  for (const match of matchesData) {
    try {
      ...
      errors++;
      if (errors < 5) console.error(`   Error inserting agency match:`, error.message);
    }
  }
