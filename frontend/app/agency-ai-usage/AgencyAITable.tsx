...
export default function AgencyAITable({ agencies, hierarchy = [] }: AgencyAITableProps) {
  ...
  const renderHierarchyAgencyRow = (agency: FederalOrganizationWithAgencies['agencies'][0], index: number, depth: number = 1) => (
    <tr
      ...
      className={`cursor-pointer hover:bg-gov-slate-100 hover:border-l-4 hover:border-gov-navy-600 transition-all bg-gov-slate-25`}
    >
      <td className="px-4 py-3 text-sm font-medium text-gov-navy-900" style={{ paddingLeft: `${1 + depth * 1.5}rem` }}>
      ...
      <td className="px-4 py-3 text-sm">
        ...
        {agency.llmName && (
          <div className="text-xs text-gov-slate-600 mt-1">{agency.llmName}</div>
        )}
      </td>
      <td className="px-4 py-3 text-sm">
    ...
    return (
      <React.Fragment key={`org-${org.id}`}>
        ...
        <tr
          ...
          className={`${isDepartment ? 'bg-gov-slate-100' : 'bg-gov-slate-50'} hover:bg-gov-slate-100 cursor-pointer border-b border-gov-slate-200`}
        >
          <td colSpan={6} className="px-4 py-3" style={{ paddingLeft: `${1 + depth * 1.5}rem` }}>
            <div className="flex items-center gap-3">
              {(hasDirectAgencies || hasChildren) && (
                <button className="p-0.5 rounded hover:bg-gov-slate-200">
                  {isExpanded ? (
                    <ChevronDown className="w-4 h-4 text-gov-slate-600" />
                  ) : (
                    <ChevronRight className="w-4 h-4 text-gov-slate-600" />
                  )}
                </button>
              )}
              <div className="flex items-center gap-2">
                {isDepartment ? (
                  <Landmark className="w-4 h-4 text-gov-navy-600" />
                ) : (
                  <Building2 className="w-4 h-4 text-gov-navy-500" />
                )}
                <span className={`font-semibold ${isDepartment ? 'text-gov-navy-900' : 'text-gov-navy-800'}`}>
                  {org.abbreviation || org.name}
                </span>
                {org.abbreviation && (
                  <span className="text-sm text-gov-slate-600">
                    - {org.name}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 ml-auto">
                ...
                {aggregatedStats.withCoding > 0 && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-ai-blue-light text-ai-blue-dark">
  ...
  const renderAgencyRow = (agency: AgencyAIUsage, index: number, isNested: boolean = false) => (
    <tr
      key={agency.id}
      onClick={() => router.push(`/agency-ai-usage/${agency.slug}`)}
      className={`cursor-pointer hover:bg-gov-slate-100 hover:border-l-4 hover:border-gov-navy-600 transition-all ${
        isNested ? 'bg-gov-slate-25' : (index % 2 === 0 ? 'bg-white' : 'bg-gov-slate-50/30')
      }`}
    >
      <td className={`px-4 py-3 text-sm font-medium text-gov-navy-900 ${isNested ? 'pl-10' : ''}`}>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-gov-navy-700 bg-white border border-gov-slate-300 rounded-md hover:bg-gov-slate-50 transition-colors"
